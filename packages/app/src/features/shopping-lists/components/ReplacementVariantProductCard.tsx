...
export default function ReplacementVariantProductCard(props: ProductCardProps) {
  const { product, hideAddedBadge } = props;
  const imagePath = getImagePath(product.cover as IFile);
  const { isItemPresent, getItem, replaceShoppingItem, removeExtraShoppingItem, updateQuantity, hideOffcanvas, getOriginalItem } = useMarkShoppingDoneActions();
  const { shoppingListId, currentItemToReplace } = useMarkShoppingDoneState();
  const isPresentInList = isItemPresent(product.id);
  const item = getItem(product.id);
  const isExtra = isPresentInList === "extra";

  const replaceItem = useCallback(() => {
    if (!currentItemToReplace?.id) {
      return;
    }
    ...
    hideOffcanvas();
  }, [currentItemToReplace?.id, replaceShoppingItem, product, getOriginalItem, shoppingListId, hideOffcanvas]);
    ...
    <div className="bp-product_item_card">
      <div className="bp-product_item_card__main">
        ...
        <div className="bp-product_item_card__actions">
          {!isPresentInList && (
            <Button variant="outline-primary" onClick={() => replaceItem()}>
              Replace with this
            </Button>
          )}
